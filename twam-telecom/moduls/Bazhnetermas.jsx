...
import { db } from "./firebase"; // 🔗 փոխիր քո ուղուն
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (docSnap.exists()) {
                    const fetchedTabs = docSnap.data().items;
                    setTabs(fetchedTabs);
                        ...
                        {activeTabObj?.content.map((item, index) => (
                            <div key={index}>
                                <button
                                    ...
                                    className={`text-[30px] font-[600] mb-[4px] w-full text-left border-none border-b-2 transition-colors duration-300 ${openIndexes[index]
