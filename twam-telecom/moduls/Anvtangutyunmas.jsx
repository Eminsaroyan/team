...
import { db } from "./firebase"; // Ձեր firebase config ֆայլից
...
export default function Anvtangutyunmas() {
    ...
    useEffect(() => {
        async function fetchTabs() {
            try {
                // Ուշադրություն՝ այստեղ ճիշտ է հավաքած path-ը՝ collection և document անուններով
                ...
                if (docSnap.exists()) {
                    ...
                    const fetchedTabs = data.bajin; // Դաշտը 'bajin'-ն է, ոչ թե 'tabs'
                    if (!fetchedTabs || !Array.isArray(fetchedTabs) || fetchedTabs.length === 0) {
                        ...
                        {activeTabObj?.content?.map((item, index) => (
                            <div key={index}>
                                <button
                                    ...
                                    className={`text-[30px] font-[600] mb-[15px] w-full text-left border-none border-b-2 pr-[60px] transition-colors duration-300 ${
                                        openIndexes[index]
